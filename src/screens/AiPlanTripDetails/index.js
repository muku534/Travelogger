...
const AiPlanTripDetails = ({navigation}) => {
  ...
  const {
    ...
    coordinates,
    tripDays,
  } = useSelector(state => state.tripDetails);
  ...
  const calculateDistances = () => {
    ...
    for (let i = 0; i < tripDays.length; i++) {
      ...
      for (let j = 0; j < places.length - 1; j++) {
        ...
        const originName =
          places[j]?.name || places[j]?.title || 'Unnamed Place';
        const destinationName =
          places[j + 1]?.name || places[j + 1]?.title || 'Unnamed Place';
        ...
        if (
          ...
          logger.info(
            `Missing coordinates for ${originName} or ${destinationName}`,
          );
          continue;
        }
              ...
              sections: {
                hotels: day.items
                  ...
                  .map(hotel => ({
                    ...
                    photos:
                      hotel.photos && Array.isArray(hotel.photos)
                    ...
                    contact: {
                      ...
                      website:
                        hotel.contact?.website === 'N/A'
                          ? ''
                          : hotel.contact?.website || hotel.website === 'N/A'
                          ? ''
                          : hotel.website || '',
                      googleMapsUrl:
                        hotel.contact?.googleMapsUrl ||
                        hotel.googleMapsUrl ||
                        '',
                    },
                    operatingHours: {
                      isOpen:
                        hotel.operatingHours?.isOpen || hotel.isOpen || false,
                      periods: hotel.operatingHours?.periods
                        ? Object.values(
                            hotel.operatingHours.periods.reduce(
                              (acc, period) => {
                        ...
                        : hotel.openDay && hotel.openTime
                        ? [{day: hotel.openDay, hours: hotel.openTime}]
                        : [],
                    },
                  ...
                  .map(activity => ({
                    ...
                    location: {
                      ...
                      coordinates: activity.location?.coordinates
                        ...
                        : [],
                      placeId:
                        activity.location?.placeId || activity.placeId || null,
                    },
                    ...
                    photos:
                      activity.photos && Array.isArray(activity.photos)
                    ...
                    contact: {
                      ...
                      website:
                        activity.contact?.website === 'N/A'
                          ? ''
                          : activity.contact?.website ||
                            activity.website === 'N/A'
                          ? ''
                          : activity.website || '',
                      googleMapsUrl:
                        activity.contact?.googleMapsUrl ||
                        activity.googleMapsUrl ||
                        '',
                    },
                    operatingHours: {
                      isOpen:
                        activity.operatingHours?.isOpen ||
                        activity.isOpen ||
                        false,
                      periods: activity.operatingHours?.periods
                        ? Object.values(
                            activity.operatingHours.periods.reduce(
                              (acc, period) => {
                                acc[period.day] = period; // Keep only the last entry per day
                                return acc;
                              },
                        ...
                        : activity.openDay && activity.openTime
                        ? [{day: activity.openDay, hours: activity.openTime}]
                        : [],
                    },
                    metadata: {
                      tags: activity.tags || [],
                ...
                restaurants: day.items
                  ...
                  .map(restaurant => ({
                    ...
                    photos:
                      restaurant.photos && Array.isArray(restaurant.photos)
                    ...
                    contact: {
                      phone:
                        restaurant?.contact?.phone || restaurant.phone || '',
                      email:
                        restaurant?.contact?.email || restaurant.email || '',
                      website:
                        restaurant?.contact?.website === 'N/A'
                          ? ''
                          ...
                          : restaurant.website || '',
                      googleMapsUrl:
                        restaurant?.contact?.googleMapsUrl ||
                    ...
                    operatingHours: {
                      ...
                      periods: restaurant.operatingHours?.periods
                        ? Object.values(
                            restaurant.operatingHours.periods.reduce(
                              (acc, period) => {
                                acc[period.day] = period; // Keep only the last entry per day
                                return acc;
                              },
                        ...
                        : restaurant.openDay && restaurant.openTime
                        ? [
                            {
                              day: restaurant.openDay,
                              hours: restaurant.openTime,
                            },
                          ]
                        : [],
                    },
          ...
          itinerary: {
            ...
            days: tripDays.map((day, index) => ({
              ...
              sections: {
                hotels: (
                  day.sections?.hotels ||
                  day.items?.filter(item => item.category === 'Hotel')
                ).map(hotel => ({
                  id: hotel.id || null,
                  type: 'hotel',
                  ...
                  contact: {
                    ...
                    website:
                      hotel.contact?.website === 'N/A'
                        ...
                        : hotel.website || '',
                    googleMapsUrl:
                      hotel.contact?.googleMapsUrl || hotel.googleMapsUrl || '',
                  },
                  operatingHours: {
                    isOpen:
                      hotel.operatingHours?.isOpen || hotel.isOpen || false,
                    periods: hotel.operatingHours?.periods
                      ? Object.values(
                          hotel.operatingHours.periods.reduce((acc, period) => {
                            acc[period.day] = period; // Keep only the last entry per day
                            return acc;
                          }, {}),
                        )
                ...
                activities: (
                  day.sections?.activities ||
                  day.items?.filter(item => item.category === 'Activity')
                ).map(activity => ({
                  type: 'activity',
                ...
                restaurants: (
                  day.sections?.restaurants ||
                  day.items?.filter(item => item.category === 'Restaurant')
                ).map(restaurant => ({
                  type: 'restaurant',
                  title: restaurant.title || restaurant.name || '',
                  description:
                  ...
                  operatingHours: {
                    ...
                    periods: restaurant.operatingHours?.periods
                      ...
                      : restaurant.openDay && restaurant.openTime
                      ? [{day: restaurant.openDay, hours: restaurant.openTime}]
                      : [],
                  },
                  metadata: {
                    tags: restaurant.tags || [],
      ...
      switch (actionType) {
        ...
        case 'website':
          ...
          if (!websiteUrl || websiteUrl === 'N/A') {
            logger.warn(
              "Website URL is missing or 'N/A'. Redirecting to Google homepage.",
            );
  ...
  return (
    <View style={{flex: 1, backgroundColor: COLORS.white}}>
      <StatusBar
        ...
        barStyle={activeTab === 'List' ? 'light-content' : 'dark-content'}
      />
      <ScrollView
        ...
        <View style={styles.container}>
          {activeTab === 'List' ? (
            ...
            <MapView
              ...
              style={[
                {width: wp(100), height: isTablet ? hp(50) : hp(30)},
                {paddingTop: insets.top},
              ]}
              initialRegion={{
                latitude:
                  coordinates?.[0] ??
                  tripDays?.[0]?.items?.[0]?.location?.coordinates?.[0] ??
                  22.3193,
                longitude:
                  coordinates?.[1] ??
                  tripDays?.[0]?.items?.[0]?.location?.coordinates?.[1] ??
                  114.1694,
                latitudeDelta: 0.1,
                longitudeDelta: 0.1,
              }}
                ...
                return places.map((place, index) => {
                  ...
                  return (
                    <Marker
                      ...
                      <Image
                        source={getMarkerImage(place.type)}
            ...
            <TouchableOpacity
              ...
              <View style={styles.fullButton}>
                <Text
                  style={[
                    styles.toggleText,
                    activeTab === 'Map' ? styles.activeText : {},
                  ]}>
                  Map
                </Text>
              </View>
          ...
          <View style={styles.tripInfo}>
            ...
            <Text style={styles.description}>
              Plan your Trip day by day. Add Hotels, Restaurants, and Activities
              to create a Perfect Itinerary
            </Text>
          ...
          <FlatList
            ...
            renderItem={({item, index}) => {
              ...
              if (item?.items && Array.isArray(item.items)) {
                ...
                hotels = item.items.filter(
                  place =>
                    place.type?.toLowerCase() === 'hotel' ||
                    place.category === 'Hotel',
                );
                activities = item.items.filter(
                  place =>
                    place.type?.toLowerCase() === 'activity' ||
                    place.category === 'Activity',
                );
                restaurants = item.items.filter(
                  place =>
                  ...
                  <TouchableOpacity
                    ...
                    <View>
                      <Text style={styles.dayTitle}>
                        Day {index + 1}: {formatDisplayDate(item.day)}
                      </Text>
                      <Text
                        style={{
                  ...
                  {expanded === index && (
                    <View style={styles.optionsContainer}>
                      ...
                      <OptionButton
                        title="Add Hotel"
                        Icon={SVGS.AIHOTEL}
                        onPress={() => handleAddItem('hotel', index)}
                      />
                      {hotels.map((hotel, hotelIndex) => (
                        <TouchableOpacity
                          key={hotelIndex}
                          onPress={() => handleItemPress(hotel, index)}
                          activeOpacity={0.7}>
                          <View>
                            <View style={styles.placeContainer}>
                              ...
                              <View style={styles.placeDetails}>
                                <Text
                                  ...
                                  numberOfLines={1}>
                                  {hotel.title || hotel.name || 'Unnamed Hotel'}
                                </Text>
                                <View
                                  style={{
                                    flexDirection: 'row',
                              ...
                              <View style={styles.distanceContainer}>
                                ...
                                {(() => {
                                  const currentHotelName =
                                    ...
                                    'Unnamed Hotel';
                                  const nextHotelName =
                                    hotels[hotelIndex + 1]?.name ||
                                    hotels[hotelIndex + 1]?.title ||
                                    'Unnamed Hotel';
                                  return (
                                    <>
                                      <Text style={styles.distanceText}>
                                        ⏱{' '}
                                        {distances[
                                          `${currentHotelName}-${nextHotelName}`
                      ...
                      <OptionButton
                        ...
                        Icon={SVGS.ACTIVITY}
                        isImage={true}
                        onPress={() => handleAddItem('activity', index)}
                      />
                      {activities.map((activity, activityIndex) => (
                        <TouchableOpacity
                          key={activityIndex}
                          onPress={() => handleItemPress(activity, index)}
                          activeOpacity={0.7}>
                          <View>
                            <View style={styles.placeContainer}>
                              <View style={styles.imageContainer}>
                              ...
                              <View style={styles.distanceContainer}>
                                ...
                                {(() => {
                                  ...
                                  const nextActivityName =
                                    activities[activityIndex + 1]?.name ||
                                    activities[activityIndex + 1]?.title ||
                                    'Unnamed Activity';
                      ...
                      <OptionButton
                        ...
                        Icon={SVGS.RESTAURANTS}
                        onPress={() => handleAddItem('restaurant', index)}
                      />
                      {restaurants.map((restaurant, restaurantIndex) => (
                        <TouchableOpacity
                          key={restaurantIndex}
                          onPress={() => handleItemPress(restaurant, index)}
                          activeOpacity={0.7}>
                          <View>
                            <View style={styles.placeContainer}>
                              <View style={styles.imageContainer}>
                              ...
                              <View style={styles.distanceContainer}>
                                {(() => {
                                  ...
                                  const nextRestaurantName =
                                    restaurants[restaurantIndex + 1]?.name ||
                                    restaurants[restaurantIndex + 1]?.title ||
                                    'Unnamed Restaurant';
                                  return (
                                    <>
                                      <Text style={styles.distanceText}>
                                        ⏱{' '}
                                        {distances[
                                          `${currentRestaurantName}-${nextRestaurantName}`
                                        ]?.distance || 'Calculating...'}
                                      </Text>
                                      <Text style={styles.timeText}>
                                        📏{' '}
      ...
      <RBSheet
        ...
        <View>
          ...
          <TouchableOpacity
            ...
            onPress={() => handlePlaceAction(selectedItem, 'viewPlace')}>
            <Ionicons
              name="location-outline"
              size={isTablet ? wp(3) : wp(6)}
          ...
          <TouchableOpacity
            style={styles.bottomSheetButton}
            onPress={() => handlePlaceAction(selectedItem, 'directions')}>
            <Ionicons
          ...
          <TouchableOpacity
            style={[styles.bottomSheetButton]}
            onPress={() => handlePlaceAction(selectedItem, 'website')}>
            <Ionicons
              name="globe-outline"
              size={isTablet ? wp(3) : wp(6)}
              color={COLORS.darkgray}
          ...
          <TouchableOpacity
            style={[styles.bottomSheetButton, {borderBottomWidth: 0}]}
            ...
            <Ionicons
              ...
              color={COLORS.red}
            />
            <Text style={[styles.bottomSheetText, {color: COLORS.red}]}>
  ...
  <View style={styles.optionButton}>
    <View style={{flexDirection: 'row', alignItems: 'center'}}>
      {isImage ? (
        <Image
          source={Icon}
          style={{width: hp(3), height: hp(2.6), marginRight: wp(2)}}
          resizeMode="contain"
        />
      ) : (
...
const styles = StyleSheet.create({
  ...
  dayTitle: {
    fontFamily: fontFamily.FONTS.bold,
    color: COLORS.darkgray,
    fontSize: isTablet ? hp(2.2) : hp(1.8),
  },
  optionsContainer: {
    padding: isTablet ? wp(1) : wp(2.5),
  },
  optionButton: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  ...
  placeImage: {
    width: isTablet ? wp(8) : wp(15),
    height: isTablet ? wp(8) : wp(15),
    borderRadius: wp(2),
    marginRight: wp(3),
  },
  placeDetails: {
  ...
  input: {
    flex: 1,
    paddingLeft: isTablet ? wp(0) : wp(2),
    color: COLORS.darkgray,
    fontFamily: fontFamily.FONTS.Medium,
    fontSize: hp(2),
  },
  reportModelContainer: {
    backgroundColor: COLORS.white,
    width: isTablet ? wp(50) : wp(95),
    borderRadius: isTablet ? wp(1) : wp(2.2),
